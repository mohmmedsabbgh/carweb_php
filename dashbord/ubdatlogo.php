...
$ID = $_GET['id'];
$sql = "SELECT * FROM logowebsite WHERE id=$ID";
$resUPD = mysqli_query($con, $sql);
$DATA = mysqli_fetch_array($resUPD);


if (isset($_POST['editlogo'])) {
    $imglogo = $_FILES['photoLogo']['name'];
    $path = 'img/' . $imglogo;


    $sql = "UPDATE logowebsite SET photoLogo='$path'  WHERE id=$ID";

    $result = mysqli_query($con, $sql);

    if ($result) {
        echo '<script>alert("تم تعديل الشعار بنجاح") </script>';
            ...
            <div class="col-lg-10 col-md-6">
                <div class="left">
                    <div class="mainTitle mb-5">
                        <h2 class="text-center">التعديل على الشعار</h2>
                    </div>
                    <div class="formub" id="poupUp">
                        <form action="" method="post" enctype="multipart/form-data">
                            <div>
                                <p class=" fw-bold"><span>*</span>صورة الخبر</p>
                                <input type="file" name="photoLogo" class="form-control w-100 p-2 d-block" value="<?php echo $DATA['photoLogo']  ?>">
                            </div>
                            <div>
                                <button class="btn btn-warning mb-2 mt-3" name="editlogo" type="">حفظ التعديل</button>
                            </div>
                        </form>
                        <div class="taple">
                            <?php showlogo($con) ?>
