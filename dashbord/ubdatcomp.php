...
if(isset($_POST['editcomp'])){
  ...
  $sql = "UPDATE sucsuss SET photocump='$pathsu'WHERE id=$ID";

  $result = mysqli_query($con, $sql);

  if($result) {
      echo '<script>alert("تم تعديل الشركة بنجاح") </script>';
...
<head>
  ...
  <title>التعديل على الشركة</title>
        ...
        <div class="left">
          ...
          <div class="formub" id="poupUp">
            <form action="" method="post" enctype="multipart/form-data" class="w-100">
              <div>
                <p class=""><span>*</span>الصورة الجديدة</p>
                <input type="file" name="photosucsus" class="form-control w-100 p-2" value="<?php echo $DATA ['photocump']  ?>">
              </div>
              <div>
                <button class="btn btn-warning mt-3" name="editcomp" type="submit">حفظ التعديل </button>
              </div>
            </form>

            <div class="taple">
            <?php showAllsucsses($con) ?>
          </div>
          </div>
        </div>
      </div>
